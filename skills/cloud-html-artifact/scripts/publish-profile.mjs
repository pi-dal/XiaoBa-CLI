...
export function mergePublishProfile(args, options = {}) {
  ...
  if (!profileValue && !hasInlinePublishProfile(args)) {
    if (options.directHost) {
      ...
      const provider = directHost.provider === "direct-https"
        ? "direct-https"
        : "direct-ip";
...
export function derivePublishRuntimeProfile(args, options = {}) {
  ...
  if (!["static-dir", "direct-ip", "direct-https"].includes(cleanText(args.provider))) {
    throw new Error(
      "namespaceStrategy=agent-uid currently requires provider=static-dir, direct-ip, or direct-https"
    );
  }
  ...
  const localQaBaseUrl = cleanText(args["local-qa-base-url"])
    ...
    : "";
  if (["direct-ip", "direct-https"].includes(cleanText(args.provider)) && !localQaBaseUrl) {
    throw new Error(`provider=${cleanText(args.provider)} requires profile.localQaBaseUrl`);
  }
...
export function summarizePublishProfile(args) {
  if (
    !args.profile
    && !["catsco-central", "direct-ip", "direct-https"].includes(args.provider)
  ) return null;
  return {
    path: args.profile || "",
...
function normalizeDirectHost(value) {
  ...
  if (
    cleanText(value.provider)
    && !["direct-ip", "direct-https"].includes(cleanText(value.provider))
  ) {
    throw new Error(`unsupported direct host runtime provider: ${value.provider}`);
  }
  return value;
}
