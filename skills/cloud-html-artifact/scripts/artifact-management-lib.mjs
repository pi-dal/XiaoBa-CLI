...
export function canonicalizeManagedArtifactUrls(options) {
  const paths = resolveArtifactManagementPaths(options);
  const publicBaseUrl = canonicalPublicBaseUrl(options.publicBaseUrl);
  return withArtifactManagementLock(paths.registryPath, () => {
    const registryExisted = fs.existsSync(paths.registryPath);
    const registry = readOrMigrateRegistry(paths);
    const changedIds = [];
    for (const record of registry.artifacts) {
      const canonicalUrl = `${publicBaseUrl}/${encodeURIComponent(record.id)}/latest/`;
      if (record.url === canonicalUrl) continue;
      record.url = canonicalUrl;
      changedIds.push(record.id);
    }
    if (!registryExisted || changedIds.length > 0) {
      if (changedIds.length > 0) registry.updated_at = new Date().toISOString();
      writeStateTransactional(paths, registry, options);
    }
    return {
      changed: !registryExisted || changedIds.length > 0,
      rewritten: changedIds.length,
      artifact_ids: changedIds,
...
function canonicalPublicBaseUrl(value) {
  ...
  try {
    ...
    throw managementError("artifact_path_invalid", "publicBaseUrl must be an HTTPS URL", 400);
  }
