...
import { PUBLICATION_BRANDING } from "./publication-branding-contract.mjs";
import {
...
async function main() {
  const result = {
    ...
    publication_branding: {
      ...
      verified: { local: false, version: false, latest: false }
    },
  ...
  try {
    ...
    updatePublicationBrandingResult(result, "local", result.qa.local);
    ...
    if (args.provider === "catsco-central") {
      ...
      result.qa.version = runQa({
        input: result.version_url,
        label: "version",
        outDir: path.dirname(outPath)
      });
      if (!result.qa.version.ok) {
        throw new Error(`central version URL QA failed${result.qa.version.error ? `: ${result.qa.version.error}` : ""}`);
      }
      updatePublicationBrandingResult(result, "version", result.qa.version);
      result.qa.latest = runQaWithRetry({
        input: result.latest_url,
        label: "latest",
        outDir: path.dirname(outPath),
        attempts: 2,
        retryDelayMs: 1_200
      ...
      if (!result.qa.latest.ok) {
        throw new Error(`central latest URL QA failed${result.qa.latest.error ? `: ${result.qa.latest.error}` : ""}`);
      }
      updatePublicationBrandingResult(result, "latest", result.qa.latest);
...
function sanitizeRemoteQa(value) {
  ...
  return {
    ...
    publication_branding: sanitizePublicationBranding(value.publication_branding),
...
function runQa({ input, label, outDir }) {
  ...
  const commandArgs = [
    ...
    "--publication-branding-mode", "required",
    "--qa-stage", label
  ];
  ...
  return {
    ...
    report: qaPath,
    url: report?.url || "",
    publication_branding: sanitizePublicationBranding(report?.publication_branding),
...
function runQaWithRetry({ input, label, outDir, attempts, retryDelayMs }) {
  const limit = Math.max(1, Number(attempts || 1));
  let lastResult = null;
  for (let attempt = 1; attempt <= limit; attempt += 1) {
    lastResult = runQa({ input, label, outDir });
    lastResult.attempts = attempt;
    if (lastResult.ok || attempt === limit) return lastResult;
    sleepSync(Math.max(0, Number(retryDelayMs || 0)));
  }
  return lastResult;
}
...
function sanitizePublicationBranding(value) {
  if (!value || typeof value !== "object") return null;
  return {
    required: value.required === true,
    mode: cleanText(value.mode),
    contract_version: cleanText(value.contract_version),
    variant: cleanText(value.variant),
    footer_exception_reason: cleanText(value.footer_exception_reason),
    consistent_across_views: value.consistent_across_views === true
  };
}

function updatePublicationBrandingResult(result, stage, qa) {
  const branding = sanitizePublicationBranding(qa?.publication_branding);
  if (!branding || !branding.required || branding.contract_version !== PUBLICATION_BRANDING.contractVersion || !branding.variant || !branding.consistent_across_views) {
    throw new Error(`${stage} publication branding contract was not verified`);
  }
  const current = result.publication_branding;
  if (current.contract_version && current.contract_version !== branding.contract_version) {
    throw new Error(`publication branding contract changed between QA stages: ${current.contract_version} != ${branding.contract_version}`);
  }
  if (current.variant && current.variant !== branding.variant) {
    throw new Error(`publication branding variant changed between QA stages: ${current.variant} != ${branding.variant}`);
  }
  if (current.footer_exception_reason && current.footer_exception_reason !== branding.footer_exception_reason) {
    throw new Error("publication branding footer exception changed between QA stages");
  }
  current.contract_version = branding.contract_version;
  current.variant = branding.variant;
  current.footer_exception_reason = branding.footer_exception_reason;
  current.verified[stage] = true;
}
